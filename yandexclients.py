...
class IAMToken:
    def __init__(
        self, oauth_token: str, time_between_update: timedelta = timedelta(hours=1)
    ):
...
class SpeechRecognition:
    ...
    def __call__(self, speech: bytes, language_code: str) -> str:
        params = {"folderId": self._folder_id, "lang": language_code}
        headers = {"Authorization": f"Bearer {self._iam_token.get()}"}
        try:
            response = requests.post(
                self.__BASE_URL, params=params, data=speech, headers=headers
            )
