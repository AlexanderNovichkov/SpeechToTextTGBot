...
from yandexclients import SpeechRecognition, SpeechRecognitionException, IAMToken, Language
...
class SpeechRecognitionTGBot:
    ...
    def __language(self, update: Update, context: CallbackContext):
        keyboard = InlineKeyboardMarkup([
            [InlineKeyboardButton(lang.name, callback_data=lang.code) for lang in self._speech_recognition.LANGUAGES]
        ])
        context.bot.send_message(chat_id=update.effective_chat.id,
                                 text=f"Текущий язык для распознавания: {self.__get_current_chat_language(context).name}. \n"
                                      "Выберите новый язык",
    ...
    def __keyboard_callback_handler(self, update: Update, context: CallbackContext):
        ...
        try:
            language = next(filter(lambda lang: lang.code == language_code, self._speech_recognition.LANGUAGES), None)
        except:
    ...
    def __voice_or_audio_message(self, update: Update, context: CallbackContext):
        ...
        try:
            text = self._speech_recognition(speech, self.__get_current_chat_language(context).code)
            if len(text) == 0:
